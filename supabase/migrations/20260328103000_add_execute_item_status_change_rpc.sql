CREATE OR REPLACE FUNCTION public.execute_item_status_change(
  p_item_id text,
  p_action text,
...
DECLARE
  ...
  v_clear_fields text[] := COALESCE(p_clear_fields, ARRAY[]::text[]);
BEGIN
  ...
  v_new_status := CASE
    ...
    WHEN v_item.status = 'active' AND p_action = 'archive' THEN 'archived'::public.item_status
    ...
    WHEN v_item.status = 'paused' AND p_action = 'archive' THEN 'archived'::public.item_status
    ...
    WHEN v_item.status = 'trial' AND p_action = 'archive' THEN 'archived'::public.item_status
  ...
  IF v_resolved_effective_date IS NOT NULL
     AND p_minimum_effective_date IS NOT NULL
     AND v_resolved_effective_date < p_minimum_effective_date THEN
